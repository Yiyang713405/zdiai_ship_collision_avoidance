...
def headon_waypoint(cog_os, ang_vp_ot, distance_ot, safe_dcpa):
    ...
    print('cog_waypoint_right', cog_waypoint_right)
...
def multi_ship_scenario_waypoint(os_ship, ts_ships, safe_dcpa, safe_tcpa, sog_os_min, ts_ships_length, max_turn_angle):
    ...
    for i in range(len(ts_ships)):
        ...
        angle_vp_to, angle_v_to = real_angle(x, y, sog, cog, x_os, y_os, sog_os, cog_os)
        if -6 <= angle_vp <= 112.5 or (112.5 < angle_vp_to < 180 or -180 < angle_vp_to < -112.5):
            f_num += 1
        if ((t < safe_tcpa and -6 <= angle_vp <= 112.5 and d < safe_dcpa) or
            (112.5 < angle_vp_to < 180 and t < safe_tcpa and d < safe_dcpa) or
            ...
            if 0 < t < 0.8 * safe_tcpa and 0 < d < 0.8 * safe_dcpa:
              ...
              (-90 < angle_vp < 90 and (-180 <= angle_v <= -174 or 174 <= angle_v <= 180)) and not
              (-180 < angle_vp_to < -112.5 or 112.5 < angle_vp_to < 180)):  # 处于本船直航区间的风险目标船
    ...
    if dcpa_f < safe_dcpa and tcpa_f < safe_tcpa:
        ...
        #     waypoint = waypoint_f
        #     avoid_ship_label = avoid_ship_label_f
    elif f_num == 0 and (dcpa_b < safe_dcpa and tcpa_b < safe_tcpa):
