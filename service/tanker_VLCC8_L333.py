...
hull.SBH = 27193.9654  # (m^2)
hull.DISPV = 312618.0544  # (m^3)
...
propeller.D = 9.86  # (m)
propeller.P = 7.122  # (m)
propeller.PDR = 0.7212  # (1)
propeller.ADE = 0.448  # (1)
...
propeller.x = -153.60  # (m)
propeller.y = 0.00  # (m)
propeller.z = 0.00  # (m)

propeller.rps_serv = 10.4  # (Hz) TODO
propeller.rps_dot_serv = 10.0  # (1/s^2) rps changing rate TODO
...
rudder.SX = 24.0323  # (m^2)
rudder.SM = 112.6386  # (m^2)
rudder.ST = 136.6710  # (m^2)
rudder.B = 15.7942  # (m)
rudder.C = 8.6445  # (m)
rudder.AS = 1.827  # (1)
...
rudder.xH_pri = -0.464
rudder.xH = rudder.xH_pri * hull.LPP  # TODO
...
rudder.delta_dot_serv_deg = 15.8  # (deg/s) Rudder angle changing rate 舵角变化率


# --------------------------------------------------------------------------- #
# Vessel
# --------------------------------------------------------------------------- #
class VLCC8L333(Vessel):

    def __init__(self):
        super(VLCC8L333, self).__init__()
    ...
    def get_ode_dot(self, _, ode_y, field):
        ...
        #w0 = 1 + math.sin(0.2 * i)
        # error1 = 0.01
        # w0 = error1 * np.random.normal(0,0.1)

        # Conduct the force matrix
        mat_force = np.array([self.u, X_tot, self.v, Y_tot, self.r, N_tot], dtype=float)
        # mat_force = np.array([self.u, X_tot, self.v, Y_tot , self.r, N_tot + 20], dtype=float)
        # if 140 <= i <= 142:
            # mat_force = np.array([self.u, X_tot, self.v, Y_tot, self.r, N_tot + 20], dtype=float)
        # else:
            # mat_force = np.array([self.u, X_tot, self.v, Y_tot, self.r, N_tot], dtype=float)
    ...
    def get_ode_dotdisturb(self, _, ode_y, field):
        self.n = ode_y[0]
        self.u = ode_y[1]
        self.e = ode_y[2]
        self.v = ode_y[3]
        ...
        # M_tot = (K_H + K_P + K_R) * self.is_roll  # Total roll moment on body around body x
        # K_tot = (M_H + M_P + M_R) * self.is_pitch  # Total pitch moment on body around body y
        N_tot = (N_H + N_P + N_R) * self.is_yaw + 10000000000 # Total yaw moment on body around body z
    ...
    def set_start_state(self):
        self.start_state = np.array([
            self.n, self.u, self.e, self.v, self.psi, self.r,
            self.propeller.rps, self.rudder.delta
        ], dtype=float)
...
def main():
    ...
    ship_param.u = 7.952  # (m/s)
    ship_param.psi_deg = 45  # (deg)
    ...
    ship_param.rudder.delta_cmd_deg = 30  # (deg)
    ...
    with ip_help.IpTimer():
        ...
        sim_hist_fs_rad = motion_solver.solve(field_param, ship_param)
        sim_hist_fs_deg = ip_transform.hist_rad2deg(ship_param, sim_hist_fs_rad)
    print("*" * 16 + " End " + "*" * 16 + "\n")
    ...
    # Process
    sim_hist_ms_deg = ip_transform.hist_f2m(ship_param, sim_hist_fs_deg)
    # Plot
